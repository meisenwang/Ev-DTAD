...
from evlearn.inference_engines.vcf_detection_evdtad import (
    InferenceEngineVCFDetectionEvDTAD
)
...
class VCFDetectionEvDTAD(ModelBase):
    ...
    # def _print_param_counts_once(self, tag: str):
    #     # DDP 下只让 rank0 打印，避免刷屏
    ...
    #     parts = [("backbone", getattr(nets, "backbone", None)),
    #              ("encoder",  getattr(nets, "encoder", None)),
    #              ("decoder",  getattr(nets, "decoder", None))]

    #     # # EMA 可选
    #     # if hasattr(nets, "ema_backbone"):
    #     #     parts += [("ema_backbone", getattr(nets, "ema_backbone", None)),
    #     #               ("ema_encoder",  getattr(nets, "ema_encoder", None)),
    #     #               ("ema_decoder",  getattr(nets, "ema_decoder", None))]
    ...
    #     train_all = sum(x[2] for x in rows)

    #     opt_n = None
    #     if getattr(self, "_optimizers", None) is not None and getattr(self._optimizers, "main", None) is not None:
    #         opt_n = count_optimizer(self._optimizers.main)

    #     print(f"\n[PARAMS] {tag}")
    #     for name, t, tr in rows:
    #         print(f"  - {name:12s}: total={fmt(t):>8s} | trainable={fmt(tr):>8s}")
    #     line = f"  => TOTAL        : total={fmt(total_all):>8s} | trainable={fmt(train_all):>8s}"
    ...
    def train_step(self):
        ...
        # if not self._printed_params_train_once:
        #     self._print_param_counts_once("TRAIN_START")
        #     self._printed_params_train_once = True
    ...
    def construct_inference_engine(self, fuse_postproc = False):
        ...
        return InferenceEngineVCFDetectionEvDTAD(
