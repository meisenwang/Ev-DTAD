...
def _drop_fully_empty_samples(batch):
    """Remove sampler padding entries such as ``(None, None, ..., None)``."""
    return [
        sample for sample in batch
            if not (
                isinstance(sample, (list, tuple))
                and len(sample) > 0
                and all(elem is None for elem in sample)
            )
    ]
...
def default_with_simple_labels(batch):
    ...
    batch = _drop_fully_empty_samples(batch)
