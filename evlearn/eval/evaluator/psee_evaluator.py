...
def per_class_map_metrics(coco_eval, classes):
    precisions = coco_eval.eval['precision']
    # precision dims: [IoU thresholds, recall thresholds, classes, area, max dets]
    ...
    for idx, name in enumerate(classes):
        ...
        metrics[f'mAP_{sanitize_metric_name(name)}'] = float(ap)

    return metrics

def empty_map_metrics(classes):
    metrics = { k : 0 for k in COCO_METRICS }
    metrics['mAP'] = 0
    metrics.update({
        f'mAP_{sanitize_metric_name(name)}' : 0
            for name in classes
    })
    return metrics
...
def run_psee_evaluation(
    ...
    if n_det_objects == 0:
        return empty_map_metrics(classes)
    ...
    metrics = {
        k : float(result.stats[idx]) for (idx, k) in enumerate(COCO_METRICS)
    }
    metrics['mAP'] = metrics['AP']
    metrics.update(per_class_map_metrics(result, classes))
